...
process_prob_col <- function(station_data, prob_col, station) {
  ...
  present_years_only <- station_data %>%
    filter(year >= min_year, month >= 5, month <= 10)
...
process_parameter_model <- function(station_data, param, station) {
  ...
  no_log <- c("temp", "wind_ms", "sal", "DON_DIN", "NO3_PO4", "Si_N", "DON", "chl", "C_chl")
...
calculate_seasonal_mean <- function(data, probability_columns) {
  parameters <- c("NH4", "NO3", "sal", "temp", "TN", "PO4", "chl",
                  "DIP", "DIN", "NO3_PO4", "C_chl", "silicate")
...
create_seasonal_means_plot <- function(facet_name, label) {
  ...
  p <- ggplot(p_subset, aes(x = data, y = probability)) +
    ...
    ylab("Probability of observing<br><i>A. pseudogonyaulax</i>") +
...
#             
#             if (nrow(Coef_stations_back2_sub) > 0 &&
#                 !is.na(Coef_stations_back2_sub$sample_size) &&
#                 Coef_stations_back2_sub$sample_size > 1) {
#               
...
#                     mean_val,
#                     sd_val,
#                     Coef_stations_back2_sub$p_value
#                   ),
...
#                   bt.log(mean_val,
#                          sd_val,
#                          Coef_stations_back2_sub$sample_size)
#                 
#                 # Create a new row with the backtransformed value and replicate the original row information
#                 new_row <- data.frame(
#                   parameter = rep(each_parameter, each = 3),
#                   data = c(
#                     backtransformed_val[[1]],
#                     backtransformed_val[[4]],
#                     Coef_stations_back2_sub$p_value
#                   ),
#                   type = c("mean", "sd", "p_value"),
#                   treat = rep(each_treat, each = 3),
  ...
  function(data,
    ...
    gg <-
      ...
      scale_x_continuous(
        ...
        labels = sprintf("%02d", seq(7, 23, by = 4)),
        limits = c(2007, 2023),
        expand = c(0.01, 0.01)
      ) +
